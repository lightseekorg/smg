#!/bin/bash
# Build script for WASM guest storage hook passthrough example
# This script builds the WASM storage hook passthrough component

set -e

echo "Building WASM guest storage hook passthrough example..."

# Check if we're in the right directory
if [ ! -f "Cargo.toml" ]; then
    echo "Error: Cargo.toml not found. Please run this script from the wasm-guest-storage-hook-passthrough directory."
    exit 1
fi
...
# Output locations
WASM_MODULE="target/wasm32-wasip2/release/wasm_guest_storage_hook_passthrough.wasm"
WASM_COMPONENT="target/wasm32-wasip2/release/wasm_guest_storage_hook_passthrough.component.wasm"
...
if [ -f "$WASM_COMPONENT" ]; then
    ...
    echo ""
    echo "Usage: Pass the component file to the gateway via --storage-hook-wasm-path"
    echo "or use it as a test fixture for E2E tests."
else
