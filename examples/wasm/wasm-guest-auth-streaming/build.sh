#!/bin/bash
# Build script for WASM guest auth-streaming example
# This script simplifies the build process for the WASM middleware component

set -e

echo "Building WASM guest auth-streaming example..."

# Check if we're in the right directory
if [ ! -f "Cargo.toml" ]; then
    echo "Error: Cargo.toml not found. Please run this script from the wasm-guest-auth-streaming directory."
    exit 1
fi
...
# Output locations
WASM_MODULE="target/wasm32-wasip2/release/wasm_guest_auth_streaming.wasm"
WASM_COMPONENT="target/wasm32-wasip2/release/wasm_guest_auth_streaming.component.wasm"
...
if [ -f "$WASM_COMPONENT" ]; then
    ...
    echo '2. Deploy with body_policy: "HeadersOnly" (see README.md)'
