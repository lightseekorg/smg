...
mod tests {
    ...
    #[test]
    fn test_score_request_deserialization_array() {
        let json = r#"{
            "model": "modernbert-reranker",
            "text_1": "What is the capital of France?",
            "text_2": ["Paris is the capital.", "London is in England."]
        ...
        assert_eq!(req.model, "modernbert-reranker");
        assert_eq!(req.text_1, "What is the capital of France?");
    ...
    #[test]
    fn test_score_request_deserialization_single() {
        let json = r#"{
            "model": "modernbert-reranker",
            "text_1": "What is the capital of France?",
            "text_2": "Paris is the capital."
        }"#;
    ...
    fn test_score_response_deserialization() {
        let json = r#"{
            "object": "list",
            "data": [
                {"object": "score", "score": 9.25, "index": 0},
                {"object": "score", "score": -1.5, "index": 1}
            ],
            "model": "modernbert-reranker",
            "usage": {
                "prompt_tokens": 10,
        ...
        assert_eq!(resp.model, "modernbert-reranker");
        assert_eq!(resp.data.len(), 2);

        assert_eq!(resp.data[0].object, "score");
        assert_eq!(resp.data[0].score, 9.25);
        assert_eq!(resp.data[0].index, 0);

        assert_eq!(resp.data[1].score, -1.5);
