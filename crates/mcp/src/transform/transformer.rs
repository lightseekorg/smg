...
/// Extract image-generation fallback text from MCP content payload:
/// `[{"type":"text","text":"..."}]`.
pub fn extract_image_generation_fallback_text(value: &Value) -> Option<String> {
    value.as_array().and_then(|content| {
...
impl ResponseTransformer {
    ...
    /// the image-generation success payload shape.
    ///
    /// Input format:
    /// - `obj` is a JSON object parsed from a MCP content item:
    ///   `[{"type":"text","text":"<json-string>"}]`
    /// - expected success payload example:
    ///   `{"result":"<base64-image>","revised_prompt":"...","size":"1024x1024"}`
    ///
    /// Current rule: `result` must exist and be a string.
    fn is_image_payload_candidate(obj: &serde_json::Map<String, Value>) -> bool {
        obj.get("result").and_then(|v| v.as_str()).is_some()
    }

    /// Extract image payload from MCP `tools/call` content array payload.
    ///
    /// Input is an error
    /// Success example, where `text` contains JSON with base64 result:
    /// `[{"type":"text","text":"{\"result\":\"<base64>\"}"}]`
    ///
    /// Error example, where `text` is plain error text:
    /// `[{"type":"text","text":"Error executing tool generate_image: ..."}]`
    ///
    ...
    fn image_payload_from_wrapped_content(
        ...
        extract_image_generation_fallback_text(result).map(|msg| {
            let mut obj = serde_json::Map::new();
            obj.insert("result".to_string(), Value::String(msg));
            obj
        })
    }
...
mod tests {
    ...
    #[test]
    fn test_image_generation_transform_wrapped_content_error_maps_to_result_field() {
        let result = json!([
            {
                "type": "text",
                "text": "Error executing tool generate_image: timeout"
            }
        ]);
        ...
        match transformed {
            ResponseOutputItem::ImageGenerationCall {
                ...
                assert_eq!(
                    result.as_deref(),
                    Some("Error executing tool generate_image: timeout")
                );
            }
            _ => panic!("Expected ImageGenerationCall"),
        }
