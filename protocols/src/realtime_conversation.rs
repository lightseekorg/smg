...
///
/// Custom `Debug` impl redacts `audio` blobs and `image_url` data URIs.
#[serde_with::skip_serializing_none]
...
impl fmt::Debug for RealtimeContentPart {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::InputText { text } => f.debug_struct("InputText").field("text", text).finish(),
            Self::InputAudio { audio, transcript } => f
                .debug_struct("InputAudio")
                .field(
                    "audio",
                    &audio.as_ref().map(|a| format!("[{} bytes]", a.len())),
                )
                .field("transcript", transcript)
                .finish(),
            Self::InputImage { detail, image_url } => f
                .debug_struct("InputImage")
                .field("detail", detail)
                .field("image_url", &image_url.as_ref().map(|_| "[REDACTED]"))
                .finish(),
            Self::OutputText { text } => f.debug_struct("OutputText").field("text", text).finish(),
            Self::OutputAudio { audio, transcript } => f
                .debug_struct("OutputAudio")
                .field(
                    "audio",
                    &audio.as_ref().map(|a| format!("[{} bytes]", a.len())),
                )
...
impl ImageDetail {
    /// Resolves `Auto` to `High` per the OpenAI spec.
