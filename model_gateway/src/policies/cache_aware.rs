...
pub struct CacheAwarePolicy {
    ...
    /// - `apply_repair_page` (TreeSync repair replay) stores the
    ///   full repaired path/tokens so future tenant deltas can
    ///   resolve the advertised hash and attach tenants at the
    ///   correct node.
...
impl CacheAwarePolicy {
    ...
    fn sync_insert_tokens(&self, model_id: &str, _tokens: &[u32], path_hash: u64, tenant: &str) {
        if let Some(publisher) = self.tree_delta_publisher.read().clone() {
            let mesh_model_id = normalize_model_key(model_id);
            publisher.publish_tree_delta(
                mesh_model_id,
...
mod tests {
    ...
    fn test_apply_known_remote_insert_round_trip() {
        ...
        policy.apply_repair_page(&TreeRepairPage {
    ...
    fn test_apply_known_remote_insert_from_request_hot_path() {
        ...
        // That test seeds via `apply_repair_page`, which
