...
pub struct MemoryExecutionContext {
    ...
    /// STM: whether short-term memory condensation is active for this request.
    pub stm_enabled: bool,
    /// STM: condenser model forwarded to STMO job config when available.
...
impl MemoryExecutionContext {
    ...
    pub fn from_headers(headers: &MemoryHeaderView, runtime: &MemoryRuntimeConfig) -> Self {
        ...
        Self {
            ...
            stm_condenser_model_id: (headers.stm_enabled && runtime.enabled)
...
mod tests {
    ...
    fn memory_runtime(enabled: bool) -> MemoryRuntimeConfig {
    ...
    fn store_and_recall_requested_but_not_active_when_runtime_disabled() {
        ...
        let ctx = MemoryExecutionContext::from_headers(&headers, &memory_runtime(false));
    ...
    fn from_http_headers_reads_conversation_memory_config() {
        ...
        headers.insert(
            ...
            HeaderValue::from_static(
                r#"{"long_term_memory":{"enabled":true,"policy":"store_and_recall","subject_id":"  subject_abc  ","embedding_model_id":"  text-embedding-3-small  ","extraction_model_id":"  gpt-4.1-mini  "},"short_term_memory":{"enabled":true,"condenser_model_id":"condense-v1"}}"#,
            ),
        );

        let ctx = MemoryExecutionContext::from_http_headers(&headers, &memory_runtime(true));
        ...
        assert_eq!(ctx.stm_condenser_model_id.as_deref(), Some("condense-v1"));
    }
