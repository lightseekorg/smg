...
use smg::{
    config::{
        ...
        PostgresConfig, PrePrefillConfig, RedisConfig, RetryConfig, RouterConfig, RoutingMode,
...
struct CliArgs {
    ...
    /// Minimum total characters for pre-prefill eligibility (default: 10000)
    #[arg(long, default_value_t = 10000, help_heading = "PD Disaggregation")]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while i < raw_args.len() {
        if (raw_args[i] == "--prefill" || raw_args[i] == "--pre-prefill") && i + 1 < raw_args.len()
        {
            i += 2;
            if i < raw_args.len()
