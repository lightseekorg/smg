    ...
    tokio::spawn(async move {
        ...
        loop {
            tokio::select! {
                ...
                _ = checkpoint.tick() => {
                    // Catch health changes that bypass the broadcast channel
                    publish_workers(&context, &registry);
                }
            }
        }
...
fn collect_workers(context: &AppContext) -> Value {
    ...
    let stats = context.worker_registry.stats();
    json!({
        "workers": workers.iter().map(|w| json!({
            "url": w.url(),
            "model_id": w.model_id(),
            "worker_type": format!("{:?}", w.worker_type()),
            "connection_mode": format!("{:?}", w.connection_mode()),
            "is_healthy": w.is_healthy(),
            "load": w.load(),
            "processed_requests": w.processed_requests(),
            "circuit_breaker": format!("{:?}", w.circuit_breaker().state()),
        })).collect::<Vec<_>>(),
