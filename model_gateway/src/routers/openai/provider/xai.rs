...
use super::{types::strip_sglang_fields, Provider, ProviderError};
...
impl Provider for XAIProvider {
    ...
    fn transform_request(
        ...
        if endpoint == Endpoint::Responses {
            Self::transform_responses_input(payload);
        }
        Ok(())
    }
}
...
impl XAIProvider {
    ...
    fn transform_responses_input(payload: &mut Value) {
        let Some(input_arr) = payload.get_mut("input").and_then(Value::as_array_mut) else {
            return;
        };
