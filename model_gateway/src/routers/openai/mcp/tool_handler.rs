    ...
    pub fn mark_output_hidden(&mut self, upstream_index: usize) {
        match self.visibility_state(upstream_index) {
            ...
            OutputVisibilityState::Visible => {
                // Late redaction: providers can emit arguments events before the
                // identifying output_item payload. If we discover an item should
                // be hidden after a mapping was created, hide future forwarding.
                self.output_visibility
                    .insert(upstream_index, OutputVisibilityState::Hidden);
                warn!(
                    upstream_index,
                    "Applied late hide for already-visible output index"
                );
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn hide_after_visible_suppresses_future_forwarding() {
        let mut handler = StreamingToolHandler::with_starting_index(0);
        assert_eq!(handler.resolve_output_index_for_forwarding(11), Some(0));

        handler.mark_output_hidden(11);

        assert_eq!(handler.visibility_state(11), OutputVisibilityState::Hidden);
        assert_eq!(handler.resolve_output_index_for_forwarding(11), None);
    }
