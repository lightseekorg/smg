...
impl RequestContext {
    ...
    pub fn primary_worker_url(&self) -> Option<String> {
        self.state.workers.as_ref().and_then(|w| match w {
            WorkerSelection::Single { worker } => Some(worker.url().to_string()),
            WorkerSelection::Dual { decode, .. } => Some(decode.url().to_string()),
        })
    }
}
