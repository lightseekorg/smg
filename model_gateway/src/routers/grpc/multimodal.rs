...
pub(crate) fn assemble_multimodal_data(
    ...
    assemble_multimodal_data_impl(intermediate, client, workers, false)
}

/// Assemble multimodal data for a prefill request whose item embeddings will
/// arrive out-of-band from encode workers.
pub(crate) fn assemble_multimodal_data_after_encode(
    intermediate: MultimodalIntermediate,
    client: &GrpcClient,
    workers: Option<&WorkerSelection>,
) -> Result<MultimodalData> {
    assemble_multimodal_data_impl(intermediate, client, workers, true)
}
...
fn assemble_multimodal_data_impl(
    intermediate: MultimodalIntermediate,
    client: &GrpcClient,
    workers: Option<&WorkerSelection>,
