...
//! gRPC regular Responses streaming pipeline so WebSocket connections reuse the
//! same event-emission, storage, and persistence logic as HTTP SSE.
...
impl WsResponsesExecutor for GrpcWsResponsesExecutor {
    async fn execute_response_create(
        ...
        let ctx = &self.responses_context;
        ...
        let final_response = if has_mcp_tools {
            ...
            let chat_request = conversions::responses_to_chat(&modified_request)
                .map_err(|err| WsClientError::new("convert_request_failed", err))?;
