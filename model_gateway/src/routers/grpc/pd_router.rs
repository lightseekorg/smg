...
/// gRPC disaggregated dual-dispatch router. Serves both PD (prefill-decode) and
...
impl GrpcPDRouter {
    ...
    /// pipelines insert the encode stage (image embeddings fan out to encode
    /// workers over Mooncake) and all pipelines dispatch the prefill+decode leg
    /// via `ExecutionMode::Epd`. Completion is text-only so it has no encode
    /// stage but still uses the EPD execution path.
