...
///
/// Populated from `x-conversation-memory-config` JSON (preferred) or
/// from legacy `x-smg-memory-*` flat headers (backward compat).
    ...
    if let Some(raw) = extract_header_value(headers, &HEADER_CONVERSATION_MEMORY_CONFIG) {
        if let Ok(cfg) = serde_json::from_str::<MemoryConfigJson>(raw) {
            return ConversationMemoryConfig {
                ...
                ltm_subject_id: cfg.long_term_memory.subject_id,
                ltm_embedding_model_id: cfg.long_term_memory.embedding_model_id,
                ltm_extraction_model_id: cfg.long_term_memory.extraction_model_id,
                stm_enabled: cfg.short_term_memory.enabled,
                stm_condenser_model_id: cfg.short_term_memory.condenser_model_id,
            };
        }
    }
