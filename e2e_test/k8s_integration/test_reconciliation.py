"""Integration tests for K8s service discovery and reconciliation (PR #1039).
...
These tests require a kind cluster with the gateway deployed.
The reconciliation interval is hardcoded to 60s in the gateway (main.rs:1275),
so tests exercising reconciliation must wait ~90s for a tick to fire.
...
def _wait_for_pod_gone(name: str, timeout: int = 60):
    ...
    Raises TimeoutError if the pod still exists after timeout.
    """
    ...
    while time.time() < deadline:
        ...
        if result.returncode != 0:
            if "NotFound" in result.stderr or "not found" in result.stderr.lower():
                ...
                return
            # Non-"not found" errors are likely transient; log and retry
            logger.warning(
                "kubectl get pod %s returned error (may be transient): %s",
                name,
                result.stderr.strip(),
            )
        time.sleep(2)
...
def _poll_until(predicate, description: str, timeout: int, interval: int = 5) -> bool:
    ...
    Only transient errors (network, HTTP) are retried. Programming errors
    (KeyError, TypeError, etc.) propagate immediately to avoid masking bugs.
    """
    deadline = time.time() + timeout
    last_error = None
    attempts = 0
    while time.time() < deadline:
        ...
        logger.debug("Waiting for: %s (%ds remaining)", description, remaining)
        time.sleep(interval)
